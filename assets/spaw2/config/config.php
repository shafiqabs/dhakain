...
function serverURI($int=0){
    ...
    $requireUri = parse_url($requireUri);
    $pageURL=explode("/",$requireUri['path']);
    $val = (int)$pageURL[$int];
    return $val = 2;
...
$directory=serverURI(5);
//define("CUS_DIRECTORY", $directory);
SpawConfig::setStaticConfigItem('CUSTOM_DIR',$directory,SPAW_CFG_TRANSFER_NONE);
...
SpawConfig::setStaticConfigItem(
  ...
  array(
    ...
    array(
      'dir'     => SpawConfig::getStaticConfigValue('SPAW_DIR').'uploads/images/'.SpawConfig::getStaticConfigValue('CUSTOM_DIR').'/',
      'caption' => 'Images',
      'params'  => array(
