...
<?php $this->load->view('admin/layouts/left_menu'); ?>
  ...
  <form class="form-horizontal" name="form" id="formID4" action="<?php echo site_url($this->uri->segment(1,0).'/advertisment/delete/'.$access_slug)?>" method="post" enctype="multipart/form-data">
    <div class="right-area ">
      <div class="submit-bar persist-header" >
        <div id="message-box"><?php $this->load->view('admin/layouts/message'); ?></div>
        <div class="submit-action"><ul><li><button class="btn btn-primary" type="submit"><?php echo $this->lang->line('all_delete'); ?></button></li><li><button class="btn"><?php echo $this->lang->line('cancel'); ?></button></li></ul></div>
      </div>
            ...
            <tr class="gradeX " id="listItem_<?php echo $rows->random_id; ?>">
              ...
              <td class="center">
                <img src="<?php echo base_url();?>assets/images/arrow.png" alt="move" width="16" height="16" class="handle" />
                  <?php if($rows->status ==1){ ?><a class="publish_img" href="<?php echo site_url($this->uri->segment(1,0).'/advertisment/status/'.$access_slug.'/'.$rows->random_id); ?>"></a><?php } else if($rows->status ==0){ ?><a class="unpublish_img" rel="" id="" href="<?php echo site_url($this->uri->segment(1,0).'/advertisment/status/'.$access_slug.'/'.$rows->random_id); ?>"></a><?php } ?>
                  <a href="<?php echo site_url($this->uri->segment(1,0).'/advertisment/edit/'.$access_slug.'/'.$rows->random_id); ?>" class="edit_img" ></a>
                <a rel="<?php echo site_url($this->uri->segment(1,0).'/advertisment/delete/'.$access_slug.'/'.$rows->random_id); ?>" class="delete_img" href="javascript:void(0)"></a>
              </td>
            </tr>
  ...
  $(document).ready(function() {
    $(".sorting").sortable({
      ...
      update : function () {
        ...
        $.ajax({
          type: "POST",
          url:"<?php echo site_url('global/advertisment/ordering/'.$access_slug)?>",
          data:order,
          cache: false,
