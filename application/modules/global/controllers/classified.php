...
class Classified extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('global/classified_model'));
    ...
    function classified_view($access_slug){
        ...
        $this->storeInfo = $this->store_model->storeInformation(array('shop_group','shop_name','brand_status','setup_website','listing_type'),$access_slug);
        $data['store_name'] = $this->storeInfo[0]->shop_group;
        $data['storeInfo']=$this->storeInfo;
        $data['agent_uid'] = $agent_uid;
        $data['access_slug'] = $access_slug;
        $this->load->view('global/classified/classified_view',$data);
