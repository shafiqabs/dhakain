...
class Main_model extends CI_Model
{
    ...
    public function getSponsoredStore()
    {

        $this->db->select('shop_name,subdomain_name,domain_name,location_id,settings_name as location_name,logo');
        $this->db->from('shop_details');
        $this->db->join('users','users.uid = shop_details.agent_uid','left');
        $this->db->join('settings','settings.settings_id = shop_details.location_id','left');
        $this->db->where('shop_details.sponsored',1);
