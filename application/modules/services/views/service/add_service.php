<div class="modal hide" id="storeModal">
    ...
    <div class="modal-body">
        <form class="well shop-form" id="storeID" action="<?php echo site_url($this->uri->segment(1,0).'/store/insert')?>" method="post">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <select name="shop_type[]" class="validate[required] populate select2-offscreen" style="width:470px" multiple="" id="e8_2" tabindex="-1">
                        <?php
                        $query=$this->classified_model->categorylist("services",$category_id=0);
                        foreach ( $query->result() as $rows ){

                        ?>
                        <optgroup label="<?php  echo $rows->category_name; ?>">
                            <?php
                            $query1=$this->classified_model->categorylist("services",$rows->category_id);
            ...
            <input type="hidden" name="shop_group" id="shop_group" value="services">
    ...
    jQuery(document).ready(function(){
        ...
        $('#shopname').change(function(){
            ...
            $.ajax({
                ...
                data:'shop_name='+shop_name+"&location_id="+location_id+"&shop_group=service",
                cache: false,
                beforeSend:function(){
        ...
        $("#e8_2_get2").click(function (){
            ...
            $.ajax({
                type: "POST",
                url:"<?php echo site_url('services/service/insert')?>",
                data:data,
                cache: false,
                beforeSend:function()
                {
                ...
                success: function(val)
                {
                    if(val !=""){
                        ...
                        setTimeout(function() {
                            document.location.href="/services/store/"+val;
                        }, 2000);
