...
class Shop extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model(array('shop_model','global/classified_model'));
		$this->load->language(array('shop'));

	}

    function index()
    {
        $data['title']=$this->lang->line('shop_list');
        $data['colModel']=
        ...
        $data['sortorder']='asc';
        $data['rp']=20;
        $data['url']= site_url($this->uri->segment(1,NULL).'/shop/shop_paging/shops');
        $this->load->view($this->uri->segment(1,NULL).'/shop/listing_view',$data);
    }

    function shop_paging($shop_group)
    {
        echo $this->shop_model->storePaging($shop_group);
    }
    ...
    function edit($access_slug=NULL)
    {
        ...
        $data['shopTypeList'] = $this->settings_model->settings_under_list($id=121);

        $data['title']=$this->lang->line('add_shop');
        $data['access_slug'] = $access_slug;
        $this->load->view($this->uri->segment(1,0).'/shop/shop_view',$data);
    }
