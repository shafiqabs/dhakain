...
class settings_model extends CI_Model
{
    ...
    function slugUnify($setting_url=NULL,$parentid=0){


        $count=$this->db->where(array('settings_slug'=>$setting_url))->get('settings')->num_rows();
        if($count > 0 ){

            $this->db->select('parent_id,settings_slug');
            $this->db->where(array('settings_id'=>$parentid));
            $query=$this->db->get('settings');
            $rows=$query->result();
            if($rows[0]->settings_slug){
                return $setting_url=url_title($rows[0]->settings_slug.'-'.$setting_url);
            }else{
                return $setting_url=url_title($setting_url);
            }
