...
<form class="form-horizontal" name="form" id="formID4" action="<?php echo site_url('settings/service_template/service_single_template_add')?>" method="post">
...
<!--  start step-holder -->
     <div id="step-holder">
        ...
        <div class="step-light-left"><a href="<?php echo site_url('settings/service_template')?>"><?php echo $this->lang->line('service_multi_template'); ?></a></div>
        <div class="step-light-right">&nbsp;</div>
        <div class="step-no">2</div>
        <div class="step-dark-left"><a href="<?php echo site_url('settings/service_template/service_single_template')?>"><?php echo $this->lang->line('service_single_template'); ?></a></div>
            ...
            <div class="controls">
               <select id="product_template_manage_id" name="product_template_manage_id" style="width:350px;" >
                   ...
                   <?php
                      ...
                      $shop=$this->db->where(array('product_template_type'=>2,'template_group'=>2))->get('product_template_manage');
                      $query=$shop->result();
              ...
              <p class="help-block"><?php echo $this->lang->line('service_template_name_help'); ?></p>
            </div>
          </div>
          <div class="control-group">
            <label for="select02" class="control-label"><?php echo $this->lang->line('service_category'); ?></label>
            <div class="controls">
            <?php
			$parent_id=isset($queryup[0]->category_id)? $queryup[0]->category_id : set_value('');
            $arr=(unserialize($parent_id));
            echo $this->service_category_model->category_list($arr[0]);
            ?>
            </div>
          </div>
...
<form action="<?php site_url('settings/service_template/service_single_template_delete')?>" id="delete_list" name="delete_list">
<div class="demo_jui">
...
<tbody>
<?php
$query=$this->db->where(array('product_template_type'=>2,'template_group'=>2))->get('product_template_manage');
...
<td><?php $arr=(unserialize($rows->category_id)); echo $this->service_category_model->category_name($arr[0]); ?></td>
...
<td>
<a class="edit_img" href="<?php echo site_url('settings/service_template/service_single_template_edit/'.$rows->product_template_manage_id)?>"></a>
...
$(document).ready(function(){
	...
	$(".delete_img").click(function(){
        ...
        jConfirm(jconfirm,confirm_title,function(r) {
            if(r){
              window.location.href='<?php echo site_url('settings/service_template/service_single_template_delete');?>/'+id;
            }
        });
	})
