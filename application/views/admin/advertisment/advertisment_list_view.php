      ...
      <fieldset class="sheet">
        ...
        <div>
          ...
          <table cellpadding="0" cellspacing="0" border="0" class="display" id="grid-table">
            ...
            ?>
            <tr class="gradeX " id="listItem_<?php echo $rows->advertisment_id; ?>">
              <td><input type="checkbox" name="list[]" value="<?php echo $rows->advertisment_id; ?>" /></td>
              ...
              <td><?php if($rows->status ==1){ ?><a class="publish_img" href="<?php echo site_url($this->uri->segment(1,0).'/'.$this->uri->segment(2,0).'/status_advertisment/'.$this->uri->segment(4,0)); ?>/<?php echo $rows->access_slug; ?>"></a><?php } else if($rows->status ==0){ ?><a class="unpublish_img" rel="" id="" href="<?php echo site_url($this->uri->segment(1,0).'/'.$this->uri->segment(2,0).'/status_advertisment/'.$this->uri->segment(4,0)); ?>/<?php echo $rows->access_slug; ?>"></a><?php } ?></td>
              <td class="center">
                <img src="<?php echo base_url();?>assets/images/arrow.png" alt="move" width="16" height="16" class="handle" />
                <a href="<?php echo site_url($this->uri->segment(1,0).'/'.$this->uri->segment(2,0).'/advertisment_view/'.$this->uri->segment(4,0)); ?>/<?php echo $rows->access_slug; ?>" class="edit_img" ></a>
                <a rel="<?php echo site_url($this->uri->segment(1,0).'/'.$this->uri->segment(2,0).'/delete_advertisment/'.$this->uri->segment(4,0)); ?>/<?php echo $rows->access_slug; ?>" class="delete_img" href="javascript:void(0)"></a>
              </td>
            </tr>
  ...
  $(document).ready(function() {
    $(".sorting").sortable({
      ...
      update : function () {
        ...
        $.ajax({
          type: "POST",
          url:"<?php echo site_url($this->uri->segment(1,0).'/'.$this->uri->segment(2,0).'/sorting_advertisement/'.$this->uri->segment(4,0))?>",
          data:order,
          cache: false,
